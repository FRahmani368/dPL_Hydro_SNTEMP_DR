...
from MODELS.PGML_STemp import MLP, CudnnLstmModel, SNTEMP_flowSim
...
def main_marrmotPRMS_SNTEMP(args):
    ...
    Ts = SNTEMP_flowSim()    # SNTEMP model, that needs a flow simulator to provide sr, ss, gw
    # Ts = SNTEMP_only()    # needs observation flow
    ...
    if 0 in args["Action"]:
        ...
        for epoch in range(1, args["EPOCHS"] + 1):
            ...
            for iIter in range(1, nIterEp + 1):
                ...
                area = c_PRMS_sample[:, varC_PRMS.index("DRAIN_SQKM")].unsqueeze(-1).unsqueeze(-1).repeat(1, flowSim_total.shape[1], 1)
    ...
    if 1 in args["Action"]:
            ...
            for j in range(nrows):
                ...
                srflow = (1000 / 86400) * area * (
                        flowSim_total[:, :, 0] - flowSim_total[:, :, 3] - flowSim_total[:, :,
                                                                          4]).unsqueeze(-1).repeat(1, 1, nmul)  # Q_t - gw - ss
                ssflow = (1000 / 86400) * area * (flowSim_total[:, :, 4]).unsqueeze(-1).repeat(1, 1, nmul)  # ras
        ...
        # q_pred = flow_pred[:,:,0:1].unsqueeze(-1)
