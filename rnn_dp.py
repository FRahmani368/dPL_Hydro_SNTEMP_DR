...
class CNN1dLSTMInmodel(torch.nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.linearIn = torch.nn.Linear(nx, hiddenSize)
        self.lstm = CudnnLstm(inputSize=Nf, hiddenSize=hiddenSize, dr=dr)
        self.linearOut = torch.nn.Linear(hiddenSize, ny)
        self.gpu = 1
...
class CudnnInvLstmModel(torch.nn.Module):
    ...
    def __init__(self, *, nx, ny, hiddenSize, ninv, nfea, hiddeninv, dr=0.5, drinv=0.5):
        ...
        self.lstminv = CudnnLstmModel(nx=ninv, ny=nfea, hiddenSize=hiddeninv, dr=drinv)
        self.lstm = CudnnLstmModel(nx=nfea + nx, ny=ny, hiddenSize=hiddenSize, dr=dr)
        self.gpu = 1
...
class HBV(torch.nn.Module):
    ...
    def forward(self, x, parameters, bufftime=0, outstate=False, routOpt=False):
        ...
        if bufftime > 0:
            with torch.no_grad():
                ...
                Qsinit, SNOWPACK, MELTWATER, SM, SUZ, SLZ = initmodel(
                    xinit, parameters, bufftime=0, outstate=True, routOpt=False
                )
        else:
        ...
        parBETA = parascaLst[0][0] + parameters[:, 0] * (
            parascaLst[0][1] - parascaLst[0][0]
        )
        # parCET = parameters[:,1]
        parFC = parascaLst[1][0] + parameters[:, 1] * (
            parascaLst[1][1] - parascaLst[1][0]
        )
        parK0 = parascaLst[2][0] + parameters[:, 2] * (
            parascaLst[2][1] - parascaLst[2][0]
        )
        parK1 = parascaLst[3][0] + parameters[:, 3] * (
            parascaLst[3][1] - parascaLst[3][0]
        )
        parK2 = parascaLst[4][0] + parameters[:, 4] * (
            parascaLst[4][1] - parascaLst[4][0]
        )
        parLP = parascaLst[5][0] + parameters[:, 5] * (
            parascaLst[5][1] - parascaLst[5][0]
        )
        # parMAXBAS = parameters[:,7]
        parPERC = parascaLst[6][0] + parameters[:, 6] * (
            parascaLst[6][1] - parascaLst[6][0]
        )
        parUZL = parascaLst[7][0] + parameters[:, 7] * (
            parascaLst[7][1] - parascaLst[7][0]
        )
        # parPCORR = parameters[:,10]
        parTT = parascaLst[8][0] + parameters[:, 8] * (
            parascaLst[8][1] - parascaLst[8][0]
        )
        parCFMAX = parascaLst[9][0] + parameters[:, 9] * (
            parascaLst[9][1] - parascaLst[9][0]
        )
        # parSFCF = parameters[:,13]
        parCFR = parascaLst[10][0] + parameters[:, 10] * (
            parascaLst[10][1] - parascaLst[10][0]
        )
        parCWH = parascaLst[11][0] + parameters[:, 11] * (
            parascaLst[11][1] - parascaLst[11][0]
        )
        ...
        if routOpt is True:  # routing
            tempa = parascaLst[-2][0] + parameters[:, -2] * (
                parascaLst[-2][1] - parascaLst[-2][0]
            )
            tempb = parascaLst[-1][0] + parameters[:, -1] * (
                parascaLst[-1][1] - parascaLst[-1][0]
            )
...
class CudnnInv_HBVModel(torch.nn.Module):
    ...
    def forward(self, x, z, doDropMC=False):
        ...
        # print(Params0)
        Params = torch.clamp(
            Params0, min=0.0, max=1.0
        )  # Make all parameters into range [0,1]
