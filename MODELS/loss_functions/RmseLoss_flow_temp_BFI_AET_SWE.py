...
class RmseLoss_flow_temp_BFI_AET_SWE(torch.nn.Module):
    def __init__(self, w1=5, w2=None, w3=0.005, w4=0.001, w5=0.05, alpha=0.25, beta=1e-6):
        super(RmseLoss_flow_temp_BFI_AET_SWE, self).__init__()
        self.w1 = w1
    ...
    def forward(self, args, y_sim, y_obs, igrid):
        ...
        loss = self.w1 * loss_flow_total + self.w2 * loss_temp + \
               self.w3 * loss_BFI + self.w4 * loss_AET + self.w5 * loss_SWE
        return loss
