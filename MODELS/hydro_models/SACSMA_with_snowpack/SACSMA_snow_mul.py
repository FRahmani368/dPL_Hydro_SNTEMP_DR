...
class SACSMA_snow_Mul(torch.nn.Module):
    ...
    def forward(self, x_hydro_model, c_hydro_model, params_raw, args, muwts=None, warm_up=0, init=False, routing=False, comprout=False, conv_params_hydro=None):
        ...
        for t in range(Nstep):
            ...
            MELTWATER_storage = torch.clamp(MELTWATER_storage - refreezing, min=NEARZERO)
            tosoil = MELTWATER_storage - (params_dict["parCWH"] * SNOWPACK_storage)
            tosoil = torch.clamp(tosoil, min=0.0)
            MELTWATER_storage = torch.clamp(MELTWATER_storage - tosoil, min=NEARZERO)
            ####
            Ep = PET[t, :, :]
            ...
            UZFW_storage = torch.clamp(UZFW_storage - flux_Ru, min=NEARZERO)
            UZTW_storage = UZTW_storage + flux_Ru
            flux_Twexu = torch.clamp(UZTW_storage - uztwm, min=0.0)
            UZTW_storage = torch.clamp(UZTW_storage - flux_Twexu, min=NEARZERO)
            ...
            UZFW_storage = torch.clamp(UZFW_storage - flux_Qsur, min=NEARZERO)
            ...
            LZTW_storage = torch.clamp(LZTW_storage - flux_twexl, min=NEARZERO)
            ...
            LZFWP_storage = torch.clamp(LZFWP_storage - flux_Qbfp, min=NEARZERO)
            ...
            LZFWS_storage = torch.clamp(LZFWS_storage - flux_Qbfs, min=NEARZERO)
